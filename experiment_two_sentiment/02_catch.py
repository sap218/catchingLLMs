...
corpus = "../spotify_user_reviews/exp2/DATASET_randomised_annotated.csv"
...
output_name = "../spotify_user_reviews/exp2/DATASET_randomised_annotated_catch.csv"
stats_output_name = "../results/exp2/catch_stats.txt"
...
for post in list_of_posts:
    ...
    if matches:
        ...
        vadered = model.polarity_scores(list_of_posts[y])
        compound = vadered['compound']
        SCORE.append(compound)
        if(compound >= 0.005): SENTIMENT.append("p")
        elif(compound <= -0.005): SENTIMENT.append("n")
...
statistics.append("%s time taken to annotate (seconds): %s" % (of_interest, end_time))
